...
- 第一次使用：先看 [`doctor`](./doctor.md)、[`quickstart`](./quickstart.md)、[`sync`](./sync.md)、[`today`](./today.md)、[`next`](./next.md) 和 [`review`](./review.md)。
...
| 分组 | 命令 | 什么时候用 |
| --- | --- | --- |
| 每日控制面 | [`taskbridge today`](./today.md) | 打开跨 Provider daily hub，按 Work、Life、Inbox、Overdue 和 Recommended next 汇总。 |
| 每日控制面 | [`taskbridge next`](./next.md) | 获取跨 Provider、跨 work/life domain 的少量推荐下一步。 |
...
| 日常开始工作 | `taskbridge sync pull --all` -> `taskbridge today` -> `taskbridge next` |
...
| 同步 Provider | `taskbridge auth status` -> `taskbridge sync pull --all` |
